# ##############ver 03/28/19 1:53PM##################
# updated to ensure that its ready for eeg. Changes to cue_types list (took out distractor), and took out yellow T from probe (commented it out, can change later)
    # added the flexible v blocked mixed trials 
        # after looking at data collected around thanksgiving, we want to run a final piloting version w/ flex v blocked fand only bilateral cues and a neutral cond
...
num_trials=30 # change later to 33
...
def make_csv(filename):
    ...
    with open(filename+'.csv', mode='w') as csv_file:
        ...
        if EEGflag:
            fieldnames=['flex and blocked?','no stim','allCircsFlag','TarInDistFlag','lateralized?','neutral?','block','cue','validity','uni or bi lat?','flex or blocked?','trialNum','trial_type','corrResp','subResp','trialCorr?','RT','stim_loc(T,D)','tarinDisCond','ITI','trigs']
        else:
            ...
            for k in range(len(ThisBlock['trialsData'])): #this should be the # of trials
                ...
                if EEGflag:
                     writer.writerow({'flex and blocked?': expInfo['f and b?'],'no stim':expInfo['no stim'],
                                    ...
                                    'ITI':ThisTrial['ITI'],'trigs':ThisTrial['trigs']})
                else:
...
#                cue_target_2[0].setLineColor([1,1,0])
            ...
            if EEGflag:
                Thistrial_data= {'trialNum':(n+1),'trial_type':trial_type,'corrResp':corrKey,'subjectResp':key,'trialCorr?':trial_corr,'RT':RT, 'stim_loc':(target_circle.name,distractor_circle.name),'tarinDisCond':trial_tarInDist,'ITI':ITI,'trigs':(cue_trig,probe_trig)}
            else:
    ...
    #                continue_msg2=visual.TextStim
        if block+1==reps:
            halfway_msg=visual.TextStim(win, pos=[0,.5],units='norm',text='You are halfway done. Please call the experimenter in to continue') 
            halfway_msg.draw()
            win.flip()
