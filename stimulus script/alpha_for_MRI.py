# ### MRI adaptation of alpha paradigm 
# 03/05/19 giving 4 conditions to pilot with MRI: high valid target cue WITH distractor cues, high valid target cue WITHOUT distractor cues, high valid distractor WITHOUT target cues, low valid distractor WITHOUT target cues
 # and no high prob location manipulation-- harder to interpret using MRI BOLD in comparison to behavioral analysis. 35ish trials per block, one block ea
...
expInfo = {'subject': '', 'session': '01','refresh':50,'practice? (y/n)':'n'}
expName='alpha_pilot'
...
refresh_rate=expInfo['refresh'] #not sure what the real refresh rate is
...
num_trials=42 # should be divisible by 2
num_reps=2 # block per condition for MRI
        ...
        for n in range(len(blocks.keys())): # loop through each block
            ...
            for k in range(len(ThisBlock['trialsData'])): #this should be the # of trials
                ...
                if ThisBlock['disCue']=='disH':
                    dvalid='100%'
                elif ThisBlock['disCue']=='disL':
                    dvalid='50%'
                elif ThisBlock['disCue']=='disN':
                    dvalid='None'
...
dis_list=['disH','disL']
#Tar_distractor_list=['disN','disH'] # target WITH distractor and target WITHOUT distractor
HPLPlist=['LP']#['HP','LP'] # high prob distractor loc OR randomized prob-- changing to only randomized prob for MRI

stimList_tar=[]
for t in ['tarH']: # high valid target only
    for d in dis_list: # distractor high valid or low valid
        for p in HPLPlist:
            for n in range(num_reps):
                stimList_tar.append([t,d,p])

stimList_dis=[]
for t in ['tarN']: # NO target
    for d in dis_list: # low or high validity distractor
        for p in HPLPlist:
            for n in range(num_reps):
                stimList_dis.append([t,d,p])

#stimList_h=list(np.random.permutation(stimList_h))
#stimList_n=list(np.random.permutation(stimList_n))

stimList=stimList_tar+stimList_dis
print(stimList)
if len(stimList) > (4*num_reps): # 4 conditions 
    print('oops, you have too many blocks goin on!')
    print(stimList)
    core.quit()
...
for block in range(len(stimList)):
    ...
    #ITIpath='/Volumes/rdss_kahwang/Generate_ITIs/AlphaITIs/'
    ITIpath='Z:/Generate_ITIs/AlphaITIs/'
    ITI_rand_file=np.random.choice(os.listdir(ITIpath)) #os.listdir(ITIpath)[0]#
    
    ITI_rand_file=open(ITIpath+ITI_rand_file,'r').readlines()
    ITI_list=[]
    for i in ITI_rand_file: # making a new list of 42 ITIs for every block
        t=i.split('\n')
        t=t[0]
    ...
    for n in range(num_trials):
        ...
        ITI=make_ITI(ITI_list=ITI_list,trial_n=n)
        ...
        if thisBlock[1]=='disH' or thisBlock[1]=='disN': # if the distractor cue is valid 100% OR if there wasn't a distractor cue (we're going to act like the cue was valid but the sub doesn't know bc there was no dis cue
            if thisBlock[1]=='disH':
                disVorI='V'
            else:
                disVorI='n/a'
