...
for sub in subject_files: # insert for loop through subject list here
	...
	pattern='em_00*_Task+THHS_2019_Mar_08_*.csv'#sub+'_alpha_pilot_01_20*_*_*_*.csv'
	behav_files=[]
	for f in os.listdir(ROOT_behav):
		if fnmatch.fnmatch(f,pattern):
			behav_file=pd.read_csv(ROOT_behav+f,engine='python')
			behav_files.append(behav_file)
	...
	epCond['probe events']=mne.Epochs(raw_fe, events=events, baseline=baseline, event_id=probe_event_id, tmin=probe_tmin, tmax=probe_tmax,metadata=behav_file)
	#for event in response_event_id.keys():
		#thisID={event:response_event_id[event]}
	epCond['response events']=mne.Epochs(raw_fe, events=events, baseline=(0,None), event_id=response_event_id, tmin=response_tmin, tmax=response_tmax,metadata=behav_file)
